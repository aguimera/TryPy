...
# root.attributes('-topmost', True)  # La finestra sempre al davant
# # Carpeta con la lista de archivos CSV y Excel a combinar
# FileIn = filedialog.askopenfilename(title="Select cycles pkl file")
# if FileIn:
...
# %% Load data
DataFolder = "S:\\TriboMedData\\CharacterizationData\\TENGData\\LaserCutSamples\\03-04-2025-ExpResistancePI5878G\\"
...
for ex, dExp in dSel.groupby('ExpId'):
    ...
    for gn, df in dExp.groupby('RloadId'):
        ...
        for index, r in df.iloc[:].iterrows(): #Selects which cycles index wants to display
            Data = r.Data
